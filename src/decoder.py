...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        first_norm = self.layer_norm_1(x)
        attention_output = self.self_attention(first_norm, mask)
        x = x + attention_output
        second_norm = self.layer_norm_2(x)
        feed_forward_output = self.feed_forward(second_norm)
